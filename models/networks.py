...
def define_G(input_nc, output_nc, nz, ngf,
    ...
    if which_model_netG == 'unet_64' and where_add == 'input':
        ...
        netG = G_Unet_add_all(input_nc, output_nc, nz, 8, ngf, norm_layer=norm_layer, nl_layer=nl_layer,
                              use_dropout=use_dropout, gpu_ids=gpu_ids, upsample=upsample)
    elif which_model_netG == 'unet_256' and where_add == 'nowhere':
        netG = UNET_G(256,input_nc, output_nc, ngf)
    else:
        raise NotImplementedError(
...
# define ublock that used for generator of directly image input without latend code
...
# pix2pix generator that direct input image without latent code
